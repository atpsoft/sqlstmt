...
class TestUpdate < Minitest::Test
  def test_minimum_requirements
    assert_raises(SqlStmtError) { SqlStmtUpdate.new.table('target').to_s }
    assert_raises(SqlStmtError) { SqlStmtUpdate.new.table('target').no_where.to_s }
  end

  def test_simple
    assert_equal('UPDATE target SET blah = blee', SqlStmtUpdate.new.table('target').field('blah', 'blee').no_where.to_s)
    assert_equal('UPDATE target SET blah = blee LIMIT 3', SqlStmtUpdate.new.table('target').field('blah', 'blee').no_where.limit(3).to_s)
    assert_equal('UPDATE target SET blah = blee WHERE target_id = 1', SqlStmtUpdate.new.table('target').field('blah', 'blee').where('target_id = 1').to_s)
  end

  def test_join
    builder = SqlStmtUpdate.new.table('main m').join('other o', 'm.main_id = o.main_id')
  ...
  def test_dup
    shared_builder = SqlStmtUpdate.new.table('target')
